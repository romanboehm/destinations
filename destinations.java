...
@Command(name = "destinations", mixinStandardHelpOptions = true, version = "0.0.1",
        description = "Calculate \"common\" destinations reachable through direct train connections")
class destinations implements Callable<Integer> {

    // --local-trains-only=false --max-duration=45 8000260 8000117 8000891 8005690

    private static final String BAHN_GURU_URI_TEMPLATE = "https://api.direkt.bahn.guru/%s?localTrainsOnly=%b&v=4";

    @Parameters(description = "The destination codes (IDs) according to api.direkt.bahn.guru")
    private final List<String> destinationCodes = new ArrayList<>();

    @Option(names = {"--local-trains-only", "-l"}, description = "Exclude IC(E) connections", defaultValue = "false")
    private boolean isLocalTrainOnly;

    @Option(names = {"--max-duration", "-m"}, description = "Maximum duration in minutes", defaultValue = "60")
    private int maxDuration;

    private final HttpClient httpClient = HttpClient.newHttpClient();

    public static void main(String... args) {
        int exitCode = new CommandLine(new destinations()).execute(args);
    ...
    public Integer call() throws Exception { // your business logic goes here...
        var maps = destinationCodes.stream()
                .map(d -> destinationsToDuration(d, isLocalTrainOnly))
                .toList();
    ...
    private Map<String, Integer> findCommonDestinations(List<Map<String, Integer>> maps) {
        return maps.stream().reduce(maps.get(0), this::intersectByKeepingMaxDuration);
    }

    private Map<String, Integer> intersectByKeepingMaxDuration(Map<String, Integer> thisMap, Map<String, Integer> otherMap) {
        var intersection = new HashMap<String, Integer>();

        for (Map.Entry<String, Integer> thisEntry : thisMap.entrySet()) {
